...
pygame.display.set_caption("Aim Master - Easy Mode (Ch0kz Games)")
...
circle_radius = 35  # Больший радиус для легкого режима
circles = []
sensitivity = 1.66
...
def display_final_stats():
    ...
    high_score = settings.get('high_score_easy', 0)  # Отдельный рекорд для легкого режима
    
    # Обновляем рекорд если текущий счет выше
    if score > high_score:
        high_score = score
        settings['high_score_easy'] = high_score
    ...
    difficulty_text = small_font.render("Difficulty: Easy", True, WHITE)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                # Запускаем menu.py и закрываем aim_sec_easy.py
